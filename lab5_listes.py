...
def sauvegarder_classe(classe,fichier="classe.json"):
    try:
        with open(fichier,"w",encoding="utf-8")as f:
            json.dump(classe,f,ensure_ascii=False,indent=4)
        print("donnees sauvegardees ")
    except I0Error:
        print("Erreur de sauvegarde")
        
def charger_classe(fichier="classe.json"):
    try:
        with open(dichier,"r",encoding="utf-8") as f:
            return json.load(f)
...
def afficher_classe(classe):
    if not classe:
        ...
        return
    print(f"N | Nom  | Age  | Note ")
    
    for index, (nom, age, note) in enumerate(classe, start=1):
        print(f"{index:<4}. {nom:<15} — {age:<3} ans — note {note:<5.2f}")
...
def rechercher_etudiant(classe):
    if not classe:
        print("la classe est vide")
        return
    nom_cherche=input("nom a chercher: ").strip().lower()
    trouves=[e for e in classe if nom_cherche in e[0].lower()]
    
    if not trouves:
        print("aucun etudiant trouve")
        else:
            for nom,age,note in trouves:
